...
#include <image_node/feature.h>
#include <image_node/math_utils.h>
...
void publish(){
    ...
    sensor_msgs::ChannelFloat32 life_time;
    sensor_msgs::ChannelFloat32 u0;
    sensor_msgs::ChannelFloat32 v0;
    sensor_msgs::ChannelFloat32 u1;
    ...
    for(auto &grid_features:Curr_Feature_Buffer){
        for(auto &feature_info:grid_features){
            ids.values.push_back(feature_info.feat_id);
    ...
    for(size_t i = 0;i<curr_cam0_points_undistorted.size();i++){
        u0.values.push_back(curr_cam0_points_undistorted[i].x);
        u1.values.push_back(curr_cam1_points_undistorted[i].x);
        v0.values.push_back(curr_cam0_points_undistorted[i].y);
        v1.values.push_back(curr_cam1_points_undistorted[i].y);
    }
    feature_points->channels.push_back(ids);
    feature_points->channels.push_back(life_time);
    feature_points->channels.push_back(u0);
    feature_points->channels.push_back(v0);
...
int main(int argc, char **argv){
    ...
    feature_pub = n.advertise<sensor_msgs::PointCloud>(
            "features", 3);
